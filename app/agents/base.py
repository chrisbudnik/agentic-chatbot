...
class BaseAgent(ABC):
    ...
    async def process_before_agent_callback(
        self, user_input: str, history: List[dict], context: CallbackContext
    ...
    async def process_turn(
        ...
        # BEFORE CALLBACK
        async for event in self.process_before_agent_callback(user_input, history, context):
            yield event
        ...
        if final_answer_event:
            async for event in self.process_after_agent_callback(final_answer_event, context):
                yield event
